...
# output_hidden_states already takes care of returning the hidden states!!! So don't need to worry about that!
# In output.hidden_states!!! It should be tuple length #blocks+1 and it also includes the hidden states of after the last layer!
...
def mpt_model_forward(
    ...
    past_hidden_states: Optional[Tuple[torch.Tensor]] = None,
    past_hidden_idx: Optional[torch.Tensor] = None,
    attention_mask: Optional[torch.Tensor] = None,
    ...
    if past_hidden_states is None:  ####
        past_hidden_states = tuple([None] * len(self.blocks))
        past_hidden_idx = torch.arange(len(self.blocks))
    ...
    if past_hidden_states[0] is not None:  ####
        past_key_values_length = past_hidden_states[0].shape[2]
        seq_length_with_past = seq_length_with_past + past_key_values_length
    ...
    for i, block in enumerate(self.blocks):
        ...
        if self.gradient_checkpointing and self.training:
            ...
            outputs = block(
                ...
                layer_past_hidden_states=past_hidden_states[past_hidden_idx[i]],
...
def causal_lm_forward(
    ...
    transformer_outputs = self.transformer(
        ...
        past_hidden_idx=past_hidden_idx,
        attention_mask=attention_mask,
