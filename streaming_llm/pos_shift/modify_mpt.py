...
# Annoying thing is if we want to mask the kvs, we need to change the function signature
# But maybe we can just change how attention_mask is used to also change the which kvs are masked out??

### TODO: Currently past_key_value_mask is shape [batch_size, n_heads, past_key_value_length]
### So pass into our model tuple num_layers * [batch_size, n_heads, past_key_value_length]
...
def mpt_attention_forward(  # MptAttention
    ...
    ### key, value have the same mask so makes our lives a bit easier. Same shape as past_key_value[0]
    ...
    if past_key_value_mask is not None:
        ### past_key_value_mask is shape [batch_size, n_heads, past_key_value_length]
        ### Pad past_key_value_mask with 1s in the shape of key_states/query_states

        padded_key_value_mask = torch.cat(
            [past_key_value_mask, torch.ones((batch_size, n_heads, seq_length))],
            dim=2,
        ).reshape([batch_size, self.n_heads, 1, past_key_value_length + seq_length])

        attention_scores = attention_scores.masked_fill(
            padded_key_value_mask, torch.finfo(query_states.dtype).min
        )  ### here we broadcast across the seq_length dimension
...
def mpt_model_forward(
    ...
    ### We want to somehow either extend attention_mask use past_key_value_mask
    output_attentions = (
        output_attentions
    ...
    for block, layer_past, layer_past_mask in zip(
        self.blocks, past_key_values, past_key_value_mask
    ):
...
def causal_lm_forward(
    ...
    input_ids: Optional[torch.LongTensor] = None,
    past_key_values: Optional[Tuple[Tuple[torch.Tensor, torch.Tensor], ...]] = None,
    past_key_value_mask: Optional[
        Tuple[torch.Tensor, ...]
    ] = None,  ### key/value should have the same mask lmao
    attention_mask: Optional[torch.Tensor] = None,
    ...
    return CausalLMOutputWithCrossAttentions(
        ...
        ### Remember we have to separate out the new past_key_values when we put into the cache
        hidden_states=transformer_outputs.hidden_states,
        attentions=transformer_outputs.attentions,
    )


def enable_mpt_masked_cache(model):
